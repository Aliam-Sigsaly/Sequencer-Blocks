...
class EnvelopeSequencer:
    ...
    # Modify audio_callback to use vectorized operations
    def audio_callback(self, outdata, frames, time_info, status):
        ...
        with self.audio_lock:  # Add synchronization
            # Get current state safely
            current_note = self.current_note
            note_start = self.note_start_sample
            counter = self.counter_value

            # Calculate positions for all samples at once
            elapsed_in_note = counter - note_start + np.arange(frames)
            current_duration = self.note_durations[current_note]

            # Find where note transitions happen
            note_transitions = np.where(elapsed_in_note >= current_duration)[0]

            # Process note transitions
            if len(note_transitions) > 0:
                # Update state for next callback
                transition_idx = note_transitions[0]
                self.note_start_sample = counter + transition_idx
                self.current_note = (current_note + 1) % len(self.note_durations)

                # Update GUI in main thread
                self.root.after(0, self.update_display)

            # Calculate normalized time for all samples
            normalized_times = np.minimum(elapsed_in_note / current_duration, 1.0)

            # Calculate envelope using vectorized operations
            A = self.attack_val.get()
            D = max(self.decay_val.get(), A)

            # Calculate amplitude for all samples
            amplitudes = np.zeros(frames)
            attack_mask = normalized_times * 100 <= A
            decay_mask = (~attack_mask) & (normalized_times * 100 <= D)

            amplitudes[attack_mask] = (normalized_times[attack_mask] * 100) / A
            amplitudes[decay_mask] = 1.0 - ((normalized_times[decay_mask] * 100 - A) / (D - A))

            # Generate audio signal
            time_in_seconds = (self.sample_count + np.arange(frames)) / self.sample_rate
            frequency = self.freq_var.get()
            data = amplitudes * np.sin(2 * np.pi * frequency * time_in_seconds)

            # Update counters
            self.counter_value += frames
            self.sample_count += frames

            # Store for GUI
            if frames > 0:
                self.normalized_buffer.append(normalized_times[-1])
