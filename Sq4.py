...
class EnvelopeSequencer:
    ...
    def create_widgets(self):
        ...
        jam_frame.grid(row=6, column=0, columnspan=2, pady=10, sticky=(tk.W, tk.E))
        ...
        # Timing diagnostics
        ttk.Label(main_frame, text="Actual Sample Rate:").grid(row=7, column=0, pady=5, sticky=tk.W)
        self.rate_var = tk.StringVar(value="0 Hz")
        ttk.Label(main_frame, textvariable=self.rate_var).grid(row=7, column=1, pady=5, sticky=tk.W)

        ttk.Label(main_frame, text="Expected Sample Rate:").grid(row=8, column=0, pady=5, sticky=tk.W)
        self.expected_rate_var = tk.StringVar(value=f"{self.sample_rate} Hz")
        ttk.Label(main_frame, textvariable=self.expected_rate_var).grid(row=8, column=1, pady=5, sticky=tk.W)

        ttk.Label(main_frame, text="Timing Accuracy:").grid(row=9, column=0, pady=5, sticky=tk.W)
        self.accuracy_var = tk.StringVar(value="100%")
        ttk.Label(main_frame, textvariable=self.accuracy_var).grid(row=9, column=1, pady=5, sticky=tk.W)

        ttk.Label(main_frame, text="Normalized Time:").grid(row=10, column=0, pady=5, sticky=tk.W)
        self.normalized_var = tk.StringVar(value="0.0000")
        ttk.Label(main_frame, textvariable=self.normalized_var).grid(row=10, column=1, pady=5, sticky=tk.W)

        ttk.Label(main_frame, text="Current Note:").grid(row=11, column=0, pady=5, sticky=tk.W)
        self.note_var = tk.StringVar(value="1")
        ttk.Label(main_frame, textvariable=self.note_var).grid(row=11, column=1, pady=5, sticky=tk.W)

        # Control buttons
        control_frame = ttk.Frame(main_frame)
        control_frame.grid(row=12, column=0, columnspan=2, pady=10, sticky="ew")
